...
public class ObservationMapper {
    ...
    public ObservationDto toDto(Observation observation){
        ObservationDto dto = new ObservationDto(observation.getId(), SpeciesMapper.toDto(observation.getSpecies()), observation.getCount(), LocationMapper.toDto(observation.getLocation()), observation.getDate(),observation.getTime(),observation.getNotes());
        return dto;
    }
    ...
    public List<ObservationDto> listToDto(List<Observation> observations){
        ...
        for(int i = 0 ; i < observations.size() ; i++){
            ...
            dtoList.add(new ObservationDto(observation.getId(), SpeciesMapper.toDto(observation.getSpecies()), observation.getCount(), LocationMapper.toDto(observation.getLocation()), observation.getDate(),observation.getTime(),observation.getNotes()));
        }
